...
def duckdb_mcp() -> TransformingStdioMCPServer:
    return TransformingStdioMCPServer(
        command="uv",
        args=["tool", "run", "mcp-server-motherduck", "--db-path", ":memory:"],
        tools={},
    )


def duckdb_query_tips_tool() -> FastMCPTool:
    return FastMCPTool.from_function(
        fn=get_query_tips,
    )


def duckdb_json_tips_tool() -> FastMCPTool:
    return FastMCPTool.from_function(
        fn=get_json_tips,
    )


def get_json_tips() -> str:
    """Get helpful tips for loading and querying JSON data in DuckDB."""
    return JSON_TIPS


JSON_TIPS = """
The DuckDB JSON reader can automatically infer which configuration flags to use by analyzing the JSON file. This will work correctly in
most situations, and should be the first option attempted. In rare situations where the JSON reader cannot figure out the correct
configuration, it is possible to manually configure the JSON reader to correctly parse the JSON file.

## The `read_json` Function

The `read_json` is the simplest method of loading JSON files: it automatically attempts to figure out the correct configuration of the
JSON reader. It also automatically deduces types of columns.
...
Multiple files can be read at once by providing a glob or a list of files.
...
def get_query_tips() -> str:
    """Get helpful tips for writing DuckDB SQL queries."""
    return QUERY_TIPS
...
DuckDB executes clauses in the following order:
- FROM
- WHERE
- GROUP BY
- HAVING
...
This means it's important to perform operations before referencing them in further execution phases.
